...
export const OrderSummary = ({ checkoutValues, couponCode, register } : { 
        ...
        {checkoutValues.cartItems.map((item, index) => (
          <div key={index} className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <img src={item.image} alt={`Product ${item.id}`} className="h-12 w-12 object-cover" />
              <div>
          ...
          {['bank', 'cash', 'khalti'].map((method) => (
            <div key={method} className="flex items-center space-x-2">
              ...
              <label htmlFor={method}>
                {method === 'bank' ? 'Bank Transfer' : method === 'cash' ? 'Cash on Delivery' : 'Pay with Khalti'}
              </label>
            </div>
          ))}
        </div>
